...
def compute_metric(real_pc_path, synth_pc_path, c2c_distance=None):
    print("Computing Class-Wise M3C2 Distances...")
    ...
    classes_to_ignore = []
    print("M3C2 Medians for each class:")
    ...
    print("M3C2 Ignored classes (NaN distances only):")
    for class_number in classes_to_ignore:
        class_idx = list(classes.CLASSES_FOR_M3C2_REAL.keys())[class_number]
        class_name = classes.CLASSES_FOR_M3C2_REAL[class_idx]
        print(f"\tClass {class_name} ({class_number})")

    print("M3C2 Skipped classes (too few points):")
    ...
    for class_number, median_distance in class_wise_distances_medians:
        if class_number not in classes_to_ignore:
            #print(f"\tAdding to Mean: Class Number {class_number}, Weight {weights[weight_idx]}, Median {median_distance}")
...
def m3c2_class_wise(real_pc_path, synth_pc_path):
    print('\tM3C2 Reading & preparing data')
    ...
    print('\tM3C2 Calculating distances...')
    for class_number in tqdm(range(len(real_points_class_wise)), bar_format='\t{l_bar}{bar}'):
        ...
        if len(real_points_class_wise[class_number]) < MIN_POINTS or len(synth_points_class_wise[class_number]) < MIN_POINTS:
            ...
            print("\tM3C2 Class {} has not enough points and is skipped".format(classes.CLASSES_FOR_M3C2_REAL[list(classes.CLASSES_FOR_M3C2_REAL.keys())[class_number]]))
